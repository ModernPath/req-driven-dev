...
11. Collect the specification approval (Spec approval prompt below) — a sourced human approval of the epic's specification.
12. When the Epic Specification Gate passes and the specification approval is recorded, end by outputting the Flow 11 implementation kickoff prompt.
```
...
Specification approval (after the structural gate passes, before implementation):

The structural gate verifies the specification is complete; a human must also
approve it. Ask, record the answer with its source in the epic record
(id `SPEC-APPROVE-<EPIC-ID>`), and do not start implementation loops until it
is approved:

```text
Specification approval needed for <EPIC-ID> — <title>.
Specs: <list the specification documents and what each covers>.
Grounding: <confirm every normative claim carries a DOC:/CODE:/USER: source, or list the gaps>.
Do you approve this specification for implementation?
- approve — implementation loops may start
- request changes — what must change before approval
- defer — keep the epic at the specification stage
Approver name/role and source tag (USER:<date>) are required for approval.
```
